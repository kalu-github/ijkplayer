    ...
    if (acodec) {
        ...
        */
        ...
        if (0 == strncasecmp(mcc->codec_name, "OMX.TI.DUCATI1.", 15)) {
            ...
            opaque->n_buf_out = 1;
            PLAYER_ALOGD("using buffered output for %s", mcc->codec_name);
        }
    }
...
static int recreate_format_l(JNIEnv *env, IJKFF_Pipenode *node) {
    ...
    if (opaque->codecpar->extradata && opaque->codecpar->extradata_size > 0) {
        if ((opaque->codecpar->codec_id == AV_CODEC_ID_H264 && opaque->codecpar->extradata[0] == 1)
            ...
            if (opaque->codecpar->codec_id == AV_CODEC_ID_H264) {
                ...
                if (!opaque->bsfc) {
                    PLAYER_ALOGE("Cannot open the h264_mp4toannexb BSF!\n");
                    goto fail;
                }
            } else {
                opaque->bsfc = av_bitstream_filter_init("hevc_mp4toannexb");
                if (!opaque->bsfc) {
                    PLAYER_ALOGE("Cannot open the hevc_mp4toannexb BSF!\n");
                    goto fail;
                }
            }
            ...
            if (!convert_buffer) {
                PLAYER_ALOGE("%s:sps_pps_buffer: alloc failed\n", __func__);
                goto fail;
            }
            if (opaque->codecpar->codec_id == AV_CODEC_ID_H264) {
                if (0 !=
                    ...
                    PLAYER_ALOGE("%s:convert_sps_pps: failed\n", __func__);
                    goto fail;
                }
            } else {
                if (0 != convert_hevc_nal_units(opaque->codecpar->extradata,
                                                ...
                                                convert_buffer, convert_size,
                                                &sps_pps_size, &opaque->nal_size)) {
                    PLAYER_ALOGE("%s:convert_hevc_nal_units: failed\n", __func__);
                    goto fail;
                }
            }
    ...
    if (ffp->mediacodec_auto_rotate &&
        ...
        PLAYER_ALOGI("amc: rotate in decoder: %d\n", rotate_degrees);
        ...
        PLAYER_ALOGI("amc: rotate notify: %d\n", rotate_degrees);
...
static int reconfigure_codec_l(JNIEnv *env, IJKFF_Pipenode *node, jobject new_surface) {
    ...
    if (!opaque->acodec) {
        ...
        if (!opaque->acodec) {
            PLAYER_ALOGE("%s:open_video_decoder: create_codec failed\n", __func__);
            ret = -1;
            goto fail;
        }
    }
    ...
    if (SDL_AMediaCodec_isConfigured(opaque->acodec)) {
        if (opaque->acodec) {
            ...
            if (opaque->quirk_reconfigure_with_new_codec) {
                PLAYER_ALOGI("quirk: reconfigure with new codec");
    ...
    if (amc_ret != SDL_AMEDIA_OK) {
        PLAYER_ALOGE("%s:configure_surface: failed\n", __func__);
        ret = -1;
        goto fail;
    }
    ...
    if (amc_ret != SDL_AMEDIA_OK) {
        PLAYER_ALOGE("%s:SDL_AMediaCodec_start: failed\n", __func__);
        ret = -1;
        goto fail;
    }

    opaque->acodec_first_dequeue_output_request = true;
    PLAYER_ALOGI("%s:new acodec: %p\n", __func__, opaque->acodec);
...
static int amc_fill_frame(
    ...
    // PLAYER_ALOGE("%s: %f", __func__, (float)frame->pts);
            ...
            if (size_data && size_data_size >= 7) {
                ...
                if (change_ret < 0) {
                    ...
                    if ((opaque->codecpar->width != new_avctx->width &&
                        ...
                        PLAYER_ALOGW("AV_PKT_DATA_NEW_EXTRADATA: %d x %d\n", new_avctx->width,
                                     new_avctx->height);
    ...
    if (d->pkt_temp.data) {
        ...
        if (ffpipeline_is_surface_need_reconfigure_l(pipeline)) {
            ...
            if (!opaque->aformat_need_recreate &&
                ...
                PLAYER_ALOGI("%s: same surface, reuse previous surface\n", __func__);
                ...
                if (opaque->aformat_need_recreate) {
                    PLAYER_ALOGI("%s: recreate aformat\n", __func__);
                    ret = recreate_format_l(env, node);
                    if (ret) {
                        PLAYER_ALOGE("amc: recreate_format_l failed\n");
                        goto fail;
                    }
                ...
                if (ret != 0) {
                    PLAYER_ALOGE("%s: reconfigure_codec failed\n", __func__);
                    ret = 0;
                    goto fail;
                }
        ...
        if (input_buffer_index < 0) {
            ...
            if (!copy_size) {
                PLAYER_ALOGE("%s: SDL_AMediaCodec_getInputBuffer failed\n", __func__);
                ret = -1;
                goto fail;
            }
        }
        ...
        // PLAYER_ALOGE("%s: queue %d/%d", __func__, (int)copy_size, (int)input_buffer_size);
...
feed_input_buffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count) {
    ...
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
        ...
        if (d->bfsc_ret > 0) {
            ...
            PLAYER_ALOGE("%s: av_bitstream_filter_filter failed\n", __func__);
            ret = -1;
            goto fail;
        }
...
static int enqueue_thread_func(void *arg) {
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        PLAYER_ALOGE("%s: SetupThreadEnv failed\n", __func__);
        goto fail;
    }
    ...
    PLAYER_ALOGI("MediaCodec: %s: exit: %d", __func__, ret);
    return ret;
}
...
drain_output_buffer_l(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *dequeue_count,
    ...
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        PLAYER_ALOGE("%s:create: SetupThreadEnv failed\n", __func__);
        goto fail;
    }
    ...
    if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
        PLAYER_ALOGI("AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED\n");
        // continue;
    } else if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_FORMAT_CHANGED) {
        PLAYER_ALOGI("AMEDIACODEC__INFO_OUTPUT_FORMAT_CHANGED\n");
        SDL_AMediaFormat_deleteP(&opaque->output_aformat);
...
#ifdef FFP_SHOW_AMC_VDPS
        {
            ...
            if (0 == (opaque->benchmark_frame_count % 240)) {
                ...
                PLAYER_ALOGE("%lf fps, %lf ms/frame, %"PRIu64" frames\n",
        ...
        if (opaque->n_buf_out) {
            ...
            if (opaque->off_buf_out < opaque->n_buf_out) {
                // PLAYER_ALOGD("filling buffer... %d", opaque->off_buf_out);
                ...
                if (opaque->last_queued_pts != AV_NOPTS_VALUE &&
                    ...
                    // PLAYER_ALOGE("early picture, drop!");
                    // SDL_AMediaCodec_releaseOutputBuffer(opaque->acodec, output_buffer_index, false);
                    // goto done;
                }
                /* already sorted */
                ...
                if (pts < buf_out->pts) {
                    ...
                    for (i = opaque->off_buf_out - 1; i >= 0; i--) {
                        ...
                        if (pts > buf_out->pts) {
                            ...
                            // PLAYER_ALOGD("pts = %f", buf_out->pts);
                    ...
                    if (i < 0) {
                        // PLAYER_ALOGE("buffer too small, drop picture!");
                        if (!(bufferInfo.flags & AMEDIACODEC__BUFFER_FLAG_FAKE_FRAME)) {
...
drain_output_buffer2_l(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *dequeue_count,
    ...
    if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
        ...
        PLAYER_ALOGD("AMEDIACODEC__INFO_OUTPUT_FORMAT_CHANGED\n");
        SDL_AMediaFormat_deleteP(&opaque->output_aformat);
        opaque->output_aformat = SDL_AMediaCodec_getOutputFormat(opaque->acodec);
        if (opaque->output_aformat) {
            int width = 0;
...
static int func_run_sync(IJKFF_Pipenode *node) {
    ...
    if (!opaque->enqueue_thread) {
        PLAYER_ALOGE("%s: SDL_CreateThreadEx failed\n", __func__);
        ret = -1;
        goto fail;
    }
...
#if 0
    fallback_to_ffplay:
        PLAYER_ALOGW("fallback to ffplay decoder\n");
...
ffpipenode_config_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline, SDL_Vout *vout,
    ...
    switch (opaque->codecpar->codec_id) {
        case AV_CODEC_ID_H264:
            if (ffp->mediacodec_avc != 1 && ffp->mediacodec_all_videos != 1) {
                PLAYER_ALOGE("%s: MediaCodec: AVC/H264 is disabled. codec_id:%d \n", __func__,
                             opaque->codecpar->codec_id);
                goto fail;
            }
            switch (opaque->codecpar->profile) {
                case FF_PROFILE_H264_BASELINE:
                    PLAYER_ALOGI("%s: MediaCodec: H264_BASELINE: enabled\n", __func__);
                    break;
                case FF_PROFILE_H264_CONSTRAINED_BASELINE:
                    PLAYER_ALOGI("%s: MediaCodec: H264_CONSTRAINED_BASELINE: enabled\n", __func__);
                    break;
                case FF_PROFILE_H264_MAIN:
                    PLAYER_ALOGI("%s: MediaCodec: H264_MAIN: enabled\n", __func__);
                    break;
                case FF_PROFILE_H264_EXTENDED:
                    PLAYER_ALOGI("%s: MediaCodec: H264_EXTENDED: enabled\n", __func__);
                    break;
                case FF_PROFILE_H264_HIGH:
                    PLAYER_ALOGI("%s: MediaCodec: H264_HIGH: enabled\n", __func__);
                    break;
                case FF_PROFILE_H264_HIGH_10:
                    PLAYER_ALOGW("%s: MediaCodec: H264_HIGH_10: disabled\n", __func__);
                    goto fail;
                case FF_PROFILE_H264_HIGH_10_INTRA:
                    PLAYER_ALOGW("%s: MediaCodec: H264_HIGH_10_INTRA: disabled\n", __func__);
                    goto fail;
                case FF_PROFILE_H264_HIGH_422:
                    PLAYER_ALOGW("%s: MediaCodec: H264_HIGH_10_422: disabled\n", __func__);
                    goto fail;
                case FF_PROFILE_H264_HIGH_422_INTRA:
                    PLAYER_ALOGW("%s: MediaCodec: H264_HIGH_10_INTRA: disabled\n", __func__);
                    goto fail;
                case FF_PROFILE_H264_HIGH_444:
                    PLAYER_ALOGW("%s: MediaCodec: H264_HIGH_10_444: disabled\n", __func__);
                    goto fail;
                case FF_PROFILE_H264_HIGH_444_PREDICTIVE:
                    PLAYER_ALOGW("%s: MediaCodec: H264_HIGH_444_PREDICTIVE: disabled\n", __func__);
                    goto fail;
                case FF_PROFILE_H264_HIGH_444_INTRA:
                    PLAYER_ALOGW("%s: MediaCodec: H264_HIGH_444_INTRA: disabled\n", __func__);
                    goto fail;
                case FF_PROFILE_H264_CAVLC_444:
                    PLAYER_ALOGW("%s: MediaCodec: H264_CAVLC_444: disabled\n", __func__);
                    goto fail;
                default:
                    PLAYER_ALOGW("%s: MediaCodec: (%d) unknown profile: disabled\n", __func__,
        ...
        case AV_CODEC_ID_HEVC:
            if (ffp->mediacodec_hevc != 1 && ffp->mediacodec_all_videos != 1) {
                PLAYER_ALOGE("%s: MediaCodec/HEVC is disabled. codec_id:%d \n", __func__,
                             opaque->codecpar->codec_id);
                goto fail;
            }
        ...
        case AV_CODEC_ID_MPEG2VIDEO:
            if (ffp->mediacodec_mpeg2 != 1 && ffp->mediacodec_all_videos != 1) {
                PLAYER_ALOGE("%s: MediaCodec/MPEG2VIDEO is disabled. codec_id:%d \n", __func__,
                             opaque->codecpar->codec_id);
                goto fail;
            }
        ...
        case AV_CODEC_ID_MPEG4:
            if (ffp->mediacodec_mpeg4 != 1 && ffp->mediacodec_all_videos != 1) {
                PLAYER_ALOGE("%s: MediaCodec/MPEG4 is disabled. codec_id:%d \n", __func__,
                             opaque->codecpar->codec_id);
                goto fail;
            }
            if ((opaque->codecpar->codec_tag & 0x0000FFFF) == 0x00005844) {
                PLAYER_ALOGE("%s: divx is not supported \n", __func__);
                goto fail;
            }
        ...
        default:
            PLAYER_ALOGE("%s:create: not H264 or H265/HEVC, codec_id:%d \n", __func__,
                         opaque->codecpar->codec_id);
            goto fail;
    }

    if (strcmp(opaque->mcc.mime_type, ffp->video_mime_type)) {
        PLAYER_ALOGW("amc: video_mime_type error opaque->mcc.mime_type = %s\n",
...
ffpipenode_init_decoder_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline,
    ...
    if (!opaque->acodec_cond || !opaque->acodec_cond ||
        ...
        PLAYER_ALOGE("%s:open_video_decoder: SDL_CreateCond() failed\n", __func__);
        goto fail;
    }
    ...
    PLAYER_ALOGI("%s:use default mediacodec name: %s\n", __func__, ffp->mediacodec_default_name);
...
ffpipenode_create_video_decoder_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline,
                                                        SDL_Vout *vout) {
    PLAYER_ALOGD("ffpipenode_create_video_decoder_from_android_mediacodec()\n");
    if (SDL_Android_GetApiLevel() < IJK_API_16_JELLY_BEAN)
        return NULL;
    ...
    if (!ffpipeline_select_mediacodec_l(pipeline, &opaque->mcc) || !opaque->mcc.codec_name[0]) {
        PLAYER_ALOGE("amc: no suitable codec\n");
        goto fail;
    }
