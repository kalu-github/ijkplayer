...
#include "../ijklog/ijkplayer_log.h"
...
#if defined(__ANDROID__)
#define printf(...) PLAYER_ALOGD(__VA_ARGS__)
#endif
...
queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos,
    ...
    if (vp->bmp) {
        ...
        if (!is->viddec.first_frame_decoded) {
            PLAYER_ALOGD("Video: first frame decoded\n");
...
static int audio_decode_frame(FFPlayer *ffp) {
    ...
    if (!is->auddec.first_frame_decoded) {
        PLAYER_ALOGD("avcodec/Audio: first frame decoded\n");
...
static int stream_component_open(FFPlayer *ffp, int stream_index) {
    ...
    switch (avctx->codec_id) {
        case AV_CODEC_ID_H264 :
            meidiacodec_name = (ffp->mediacodec_avc == 2 || ffp->mediacodec_all_videos == 2)
                               ? "h264_mediacodec" : NULL;
            break;
        case AV_CODEC_ID_HEVC :
            meidiacodec_name = (ffp->mediacodec_hevc == 2 || ffp->mediacodec_all_videos == 2)
                               ? "hevc_mediacodec" : NULL;
            break;
        case AV_CODEC_ID_MPEG2VIDEO :
            meidiacodec_name = (ffp->mediacodec_mpeg2 == 2 || ffp->mediacodec_all_videos == 2)
                               ? "mpeg2_mediacodec" : NULL;
            break;
        case AV_CODEC_ID_MPEG4 :
            meidiacodec_name = (ffp->mediacodec_mpeg4 == 2 || ffp->mediacodec_all_videos == 2)
                               ? "mpeg4_mediacodec" : NULL;
            break;
        case AV_CODEC_ID_VP8 :
            meidiacodec_name = (ffp->mediacodec_vp8 == 2 || ffp->mediacodec_all_videos == 2)
                               ? "vp8_mediacodec" : NULL;
            break;
        case AV_CODEC_ID_VP9 :
            meidiacodec_name = (ffp->mediacodec_vp9 == 2 || ffp->mediacodec_all_videos == 2)
                               ? "vp9_mediacodec" : NULL;
            break;
        default :
            break;
    ...
    if ((ret = avcodec_open2(avctx, codec, &opts)) < 0) {
        av_log(NULL, AV_LOG_ERROR, "打开ffmpeg解码器失败: %d %s\n", avctx->codec_id,
               meidiacodec_name ? meidiacodec_name : "soft decoder");
...
static void ffp_log_callback_brief(void *ptr, int level, const char *fmt, va_list vl) {
    ...
    int ffplv
    __unused = log_level_av_to_ijk(level);
    PLAYER_VLOG(ffplv, LOG_TAG, fmt, vl);
}
...
static void ffp_log_callback_report(void *ptr, int level, const char *fmt, va_list vl) {
    ...
    PLAYER_ALOG(ffplv, LOG_TAG, "%s", line);
}
...
void ffp_global_init() {
    ...
    PLAYER_ALOGD("ijkmediaplayer version : %s", ijkmp_version());
...
void *ffp_set_ijkio_inject_opaque(FFPlayer *ffp, void *opaque) {
    ...
    ffp_set_option_int(ffp, FFP_OPT_CATEGORY_FORMAT, "ijkiomanager", (int64_t)(intptr_t)
    ffp->ijkio_manager_ctx);
...
int ffp_seek_to_l(FFPlayer *ffp, long msec) {
    ...
    // FIXME: 9 seekable
    av_log(ffp, AV_LOG_DEBUG, "stream_seek %"
    PRId64
    "(%d) + %"
    PRId64
...
int ffp_get_video_rotate_degrees(FFPlayer *ffp) {
    ...
    switch (theta) {
        ...
        case 360:
            ...
            break;
        default:
            PLAYER_ALOGW("Unknown rotate degress: %d\n", theta);
            theta = 0;
            break;
    }
